...
export const createClasses = async (classData: IClassConfiguration): Promise<IClassConfigurationResponse> => {
  try {
...
export const searchParents = async (query: string, page: number = 1): Promise<ParentSearchResponse> => {
  try {
    const response = await api.get<ParentSearchResponse>(`/v1/admin/parents?search=${query}&page=${page}`)
    return response.data
  } catch (error) {
    if (axios.isAxiosError(error)) {
...
export const getStaffs = async (limit: number = 10, page: number = 1, search: string = ''): Promise<StaffResponse> => {
  try {
...
export const updateStaffStatus = async (staffId: string, isActive: boolean): Promise<void> => {
  try {
...
export const deAssignSubjectToClass = async (payload: { classId: string; subjectIds: string[] }) => {
  try {
...
export const assignTutorToClass = async (classId: string, payload: { tutor: string; subjects: string[] }) => {
  try {
...
export const removeTutorOrSubjectFromClass = async (
  ...
  try {
    ...
    if (axios.isAxiosError(error)) {
      const errorMessage = error.response?.data?.message || 'Failed to remove tutor or subject from class'
      console.error('API Error:', errorMessage)
      throw new Error(errorMessage)
    }
...
export const changeClassTeacher = async (classId: string, payload: { oldTeacher: string; newTeacher: string }) => {
  try {
...
export const getPayments = async (page: number = 1, limit: number = 10, paymentStatus?: string) => {
  try {
...
export const updateExaminationStatus = async (examId: string, status: 'approve' | 'reject' | 'scheduled') => {
  try {
...
export const getAllNotices = async (search: string = '', filter: string = '', limit: number = 20) => {
  try {

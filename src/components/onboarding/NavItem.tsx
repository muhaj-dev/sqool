...
const NavItem = ({ section, index }: NavItemProps) => {
  const { activeIndex, setActiveIndex, onboarding } = useOnboarding()
  ...
  return (
    <div className="relative flex items-center">
      <button
        onClick={handleClick}
        disabled={isDisabled}
        className={cn('transition-all w-fit pb-1 hover:cursor-pointer flex flex-col items-start', {
      ...
      {isCompleted && !isCurrent && <span className="absolute -right-4 text-green-500">✓</span>}
    </div>
  )
}

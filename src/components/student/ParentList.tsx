...
import { type IParent, type ParentPaginationResponse } from "@/types";
...
const ParentList = () => {
  ...
  const fetchParents = async (page = 1, limit = 10, searchQuery?: string) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    void fetchParents(1, 10, search);
  }, [search]);

  const handlePageChange = (newPage: number) => {
    void fetchParents(newPage, pagination.pageSize, search);
  };

...
import { submitOnboardingApplication } from "@/components/Api";
...
import { SchoolInformation, OwnerInformation, SchoolSetup, OnboardingData } from "@/types/onboarding";
...
const OnboardingContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateFormData = <K extends keyof FormData>(
    section: K,
    data: Partial<FormData[K]> | (K extends 'OwnerInformation' ? OwnerInformation[] : never) | (K extends 'UploadedFiles' ? File[] : never)
  ) => {
    setFormData((prev) => {
    ...
    try {
      ...
      const requestBody = {
        ...
        ownerInformation: (Array.isArray(formData.OwnerInformation) 
          ...
          : formData.OwnerInformation ? [formData.OwnerInformation] : [])
      ...
      if (!requestBody.schoolSetup.schoolShortName) throw new Error("School short name is required");

...
const OnboardingContextProvider = ({ children }: { children: ReactNode }) => {
  ...
  const updateCompletionState = (item: string) => {
    setOnboarding(prev => prev.map(val => (val.label === item ? { ...val, isCompleted: true } : val)))
  }
  ...
  const submitApplication = async () => {
    ...
    try {
      ...
      const renamedFiles = formData.UploadedFiles.map((file, index) => {
        ...
        const newName = apiExpectedNames[index] ? `${apiExpectedNames[index]}.${originalExtension}` : file.name

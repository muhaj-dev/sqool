...
export const getSubjectColor = (name: string) => {
  ...
  return colorMap[name.toLowerCase()] || "bg-gray-100 text-gray-600 border-gray-200";
};
...
export const groupSchedulesByDay = (schedules: ClassSchedule[]) => {
  ...
  days.forEach((day) => {
    grouped[day] = schedules.filter((s) => s.day.toLowerCase() === day.toLowerCase());
  });
  return grouped;
};
...
export const getCurrentClassesInfo = (schedules: ClassSchedule[]) => {
  ...
  const completedClasses = todaySchedules.filter((s) => new Date(s.endTime) < now).length;

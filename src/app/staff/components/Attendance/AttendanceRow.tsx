...
export function AttendanceRow({ student }: AttendanceRowProps) {
  ...
  return (
    <>
      <TableRow className="hover:bg-muted/50 transition-colors">
        ...
        <TableCell>
          <Select value={record.status} onValueChange={handleStatusChange}>
            <SelectTrigger className={cn('w-[140px] capitalize', statusColors[record.status as StatusColorKey])}>
              <SelectValue />
            </SelectTrigger>
            <SelectContent>
        ...
        <TableCell>
          <div className="flex items-center gap-2">
            <Button variant="outline" size="sm" onClick={() => setShowRemarkModal(true)} className="gap-2">
              <FileText className="h-4 w-4" />
              {record.remarks ? 'Edit Remark' : 'Add Remark'}
            </Button>
            {record.remarks && (
              <span className="text-xs text-muted-foreground truncate max-w-[150px]">{record.remarks}</span>
            )}
          </div>
        </TableCell>
        <TableCell>
          <Button variant="ghost" size="sm" onClick={() => setShowInfoDialog(true)} className="gap-2">
      ...
      <StudentInfoDialog student={student} open={showInfoDialog} onOpenChange={setShowInfoDialog} />

      <RemarkModal student={student} open={showRemarkModal} onOpenChange={setShowRemarkModal} />
    </>
  )
}

...
const ParentFees = () => {
    ...
    const fetchFeeData = async () => {
      try {
        ...
        const studentList: Student[] = children.map((child: any) => ({
          ...
          admissionNumber:
            child.admissionNumber || `STD/${child._id.slice(-6)}`,
        }));
    ...
    const updatedFees = feeItems.map((fee) => {
      if (selectedFees.includes(fee.id)) {
        return {
          ...
          receiptNumber: `RCP/2024/${Math.floor(
  ...
  const getStudentTotalOwing = (studentId: string) => {
    ...
    const studentFees = feeItems.filter(
      (fee) => fee.childName === student.name
    );
    return studentFees
  ...
  <Card>
    ...
    <CardContent>
      {selectedStudent === "all" ? (
            ...
            <CardContent>
              <div className="text-2xl font-bold text-destructive">
                {formatCurrency(
                  selectedStudent === "all"
                    ? totalOwing
                    : getStudentTotalOwing(selectedStudent)
                )}
              </div>
              <p className="text-xs text-muted-foreground">
                {selectedStudent === "all"
                  ? unpaidCount
                  : getStudentUnpaidCount(selectedStudent)}{" "}
                unpaid fee
                {selectedStudent === "all"
                  ? unpaidCount !== 1
                    ? "s"
            ...
            <CardContent>
              <div className="text-2xl font-bold text-green-600">
                {formatCurrency(
                  selectedStudent === "all"
                    ? totalPaid
                    : getStudentTotalPaid(selectedStudent)
                )}
              </div>
              <p className="text-xs text-muted-foreground">
                {selectedStudent === "all"
                  ? paidCount
                  : getStudentPaidCount(selectedStudent)}{" "}
                payment
                {selectedStudent === "all"
                  ? paidCount !== 1
                    ? "s"
            ...
            <CardContent>
              <div className="text-2xl font-bold">
                {formatCurrency(
                  selectedStudent === "all"
                    ? totalFees
                    : getStudentTotalFees(selectedStudent)
                )}
              </div>
              <p className="text-xs text-muted-foreground">
                {selectedStudent === "all"
        ...
        {(selectedStudent === "all"
          ? overdueCount
          : getStudentOverdueCount(selectedStudent)) > 0 && (
          <Alert variant="destructive">
            ...
            <AlertDescription>
              ...
              {selectedStudent === "all"
                ? overdueCount
                : getStudentOverdueCount(selectedStudent)}{" "}
              overdue payment
              {selectedStudent === "all"
                ? overdueCount !== 1
                  ? "s"

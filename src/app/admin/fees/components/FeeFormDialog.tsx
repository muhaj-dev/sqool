  ...
  useEffect(() => {
    if (open) {
      if (isEditMode && selectedFee) {
        ...
        const formData = {
          class:
            typeof selectedFee?.class === "string" ? selectedFee?.class : selectedFee?.class?._id,
          session:
            typeof selectedFee?.session === "string"
              ? selectedFee?.session
              : (selectedFee?.session as any)._id,
                    ...
                    return (
                      <div key={term} className="flex gap-1">
                        <Button
                          ...
                          size="sm"
                          variant={
                            currentTerm === term ? "default" : termExists ? "secondary" : "outline"
                          }
                          onClick={() => (termExists ? setCurrentTerm(term) : addTerm(term))}
                        >
                          {term.charAt(0).toUpperCase() + term.slice(1)} Term
                          {termExists
                            ? ` - ₦${watchedTerms.find((t) => t.term === term)?.amount.toLocaleString()}`
                            : null}
                        </Button>

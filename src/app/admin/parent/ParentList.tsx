...
import { type IParent, type IStudent, type ParentPaginationResponse } from "@/types";
...
const ParentList = () => {
  ...
  const fetchParents = async (page = 1, limit = 10, searchQuery?: string, filter?: string) => {
    try {
      setLoading(true);
  ...
  useEffect(() => {
    void fetchParents(pagination.currentPage, pagination.pageSize, search, statusFilter);
  }, [search, statusFilter]);

  const handlePageChange = (newPage: number) => {
    void fetchParents(newPage, pagination.pageSize, search, statusFilter);
  };
  ...
  const RenderOccupation = ({ item }: { item: IStudent }) => {
    ...
    return (
      <p className="text-[14px] text-[#3F4946] font-medium">
        {originalParent?.occupation || "N/A"}
      </p>
    );
  };

...
export default function SignIn() {
  ...
  const handleRoleSelect = async (role: Role) => {
    ...
    const result = await completeLogin(values.email, values.password, selectedSchool!.schoolId._id, role)
  ...
  async function onSubmit(data: z.infer<typeof FormSchema>) {
    ...
    if (result.requiresOnboarding) {
      ...
      if (typeof window !== 'undefined') {
        document.cookie = `auth-token=${result.accessToken}; path=/; max-age=${60 * 60 * 24 * 7}; Secure; SameSite=Strict`
      }
